...
const { _name, _subModelName, getAttr, avoidReserveName } = require('./util');
...
class BuilderGenerator extends CommonGenerator {
  ...
  codegen(level = 0) {
    this.emitln(`public static final class Builder${this.ctx.isRequestModel ? ' extends RequestModel.Builder<' + this.realModelName + '.Builder>' : ''} {
      `, level);
...
class Generator extends CommonGenerator {
  ...
  codegen(level = 0) {
    const modelName = this.ast.modelName.lexeme;
  ...
  eachModel(level) {
    ...
    this.emitln();
    this.emitConstructor(this.ast.modelBody, realModelName, this.ctx.isRequestModel, level + 1);
    this.emitln();
  ...
  emitConstructor(ast, modelName, isRequestModel, level) {
    assert.equal(ast.type, 'modelBody');
