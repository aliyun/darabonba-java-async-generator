...
const { _name, _string, _type, _lowerFirst, _subModelName, _upperFirst, avoidReserveName, collectionType, SSE, md2Html } = require('./util');
...
class Generator extends Emitter {
    ...
    if (ast.left.type === 'id') {
      ...
      for (var i = 0; i < ast.left.propertyPath.length; i++) {
        if ((i === ast.left.propertyPath.length - 1 && ast.left.propertyPathTypes[i - 1] && ast.left.propertyPathTypes[i - 1].type === 'map') ||
          ...
          this.emit(`.put("${ast.left.propertyPath[i]}", `);
          isCollection = true;
        } else {
  ...
  visitYield(ast, level) {
    ...
    const propertyName = _name(property);
    if (propertyName === 'doSSERequest' || propertyName === 'doSSERequestReturnString') {
      const isStringSSE = propertyName === 'doSSERequestReturnString';
      const args = ast.expr.args;
      this.emit(`TeaRequest teaRequest = REQUEST.copy()`, level);
      this.emit('.setStyle(RequestStyle.');
      this.emit(`${_string(args[0].value).toUpperCase()}`);
      ...
      if (isStringSSE) {
        const action = _string(args[1].value);
        const iteratorClassName = action.split('_').map((part) => {
          return part.charAt(0).toUpperCase() + part.slice(1);
        }).join('') + 'ResponseBody';
        iteratorType = { lexeme: iteratorClassName };
        isStringType = true;
      } else {
        iteratorType = ast.expectedType;
      }
      if (iteratorType) {
        let modelName = _name(iteratorType);
        this.emitln(`${modelName}Iterator iterator = ${modelName}Iterator.create();`, level);
      } else {
        this.emitln(`com.aliyun.sdk.gateway.pop.sse.ResponseBodyIterator iterator = com.aliyun.sdk.gateway.pop.sse.ResponseBodyIterator.create();`, level);
      }
  ...
  visitObjectField(ast, level) {
    ...
    if (ast.type === 'objectField') {
      if (typeof ast.fieldName.string !== 'undefined') {
        ...
        this.emit(`new TeaPair("${ast.fieldName.lexeme}", `, level);
      }
    ...
    if (ast.type === 'map') {
      ...
      for (let i = 0; i < ast.path.length; i++) {
        ...
        if (i > 0) {
          ...
          modelName = item.lexeme;
        }
        subModelName += item.lexeme;
      }
      subModelName = _subModelName(subModelName, this.ctx.conflictModelNameMap, this.ctx.allModleNameMap);
      if (this.ast.modelName && modelName === _name(this.ast.modelName)) {
