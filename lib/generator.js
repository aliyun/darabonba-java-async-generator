...
const ResourceGenerator = require('./resource_generator');
...
class Generator {
  ...
  visit(ast, level = 0) {
    ...
    for (let i = 0; i < models.length; i++) {
      ...
      // model generator
      const model = new ModelGenerator(models[i], this);
      model.codegen();
    ...
    if (this.resources.length > 0) {
      const resource = new ResourceGenerator(this);
      resource.codegen();
      save(path.join(this.outputDir, 'Resource.java'), resource.output);
    }
  }
  ...
  async visitPom() {
    ...
    if (this.exec) {
      let mainClassPath = this.package + '.' + (this.className || 'DefaultClient');
