...
class BuilderGenerator extends CommonGenerator {
  ...
  emitConstructor(level) {
    ...
    this.emitln(`} `, level);
    this.emitln();
    this.emitln(`private Builder(${this.realModelName} model) {`, level);
    for (let i = 0; i < this.ast.nodes.length; i++) {
      let node = this.ast.nodes[i];
      this.emitln(`this.${avoidReserveName(_name(node.fieldName))} = model.${avoidReserveName(_name(node.fieldName))};`, level + 1);
    }
    this.emitln(`} `, level);
  }
