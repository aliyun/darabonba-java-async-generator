...
class BuilderGenerator extends CommonGenerator {
  ...
  codegen(level = 0) {
    const isResponseModel = this.modelName.endsWith('Response');
    this.emitln(`public static final class Builder${this.ctx.isRequestModel ? ' extends Request.Builder<' + this.realModelName + '.Builder>' : isResponseModel ? ' extends Response.Builder' : ''} {`, level);
