...
function _subModelName(name, conflictModelNameMap, allModleNameMap) {
  ...
  if (conflictModelNameMap[modelNameKey] && conflictModelNameMap[modelNameKey].includes(modelName)) {
    throw new Error(`解析失败！API 定义存在问题，路径 ${name}，${modelNameKey} 下已包含该名称 ${modelName}`);
  }
...
function _subResource(path) {
  ...
  if (tmp.length === 2) {
    const resources = tmp[1].split('&');
    resources.forEach(re => {
      resourceList.push(re.split('=')[0]);
    });
    return resourceList;
  }
  return resourceList;
...
module.exports = {
  _name, _type, _lowerFirst, _subModelName, _subResource, remove, _upperFirst, save, render, getAttr, parse, avoidReserveName, collectionType, getEnumAttr
};
