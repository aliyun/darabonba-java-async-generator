...
const { _name, _type, _lowerFirst, _subModelName, _subResource, _upperFirst, avoidReserveName, collectionType } = require('./util');
...
class Generator extends Emitter {
  ...
  visitStaticCall(ast, level) {
    ...
    var className = this.ctx.imports[ast.left.id.lexeme].className || 'DefaultClient';
  ...
  visitMethodCall(ast, level) {
    ...
    if (ast.isStatic) {
      var className = this.ctx.className || 'DefaultClient';
  ...
  visitConstructModel(ast, level) {
    ...
    if (ast.object && ast.object.fields && ast.object.fields.length > 0 && !this.ctx.exec) {
  ...
  visitReturn(ast, level) {
    ...
    if (ast.needCast && ast.expr && ast.expr.left.propertyPath && _name(ast.expr.left.propertyPath.pop()) === '_do') {
      const args = ast.expr.args;
      this.emit(`TeaRequest teaRequest = REQUEST.copy().formModel(${_name(args[7].id)}`, level);
      ...
      const resourceList = _subResource(args[3].value.string.replace(new RegExp('"', 'g'), '\\"'));
      this.ctx.resources.push(...resourceList);
      ...
      this.emitln(');');
      this.emit('ClientExecutionParams params = new ClientExecutionParams().withInput(null).withRequest(teaRequest).withOutput(', level);
  ...
  visitFieldType(value, node, modelName) {
    if (value.fieldType === 'array') {
      ...
      if (value.fieldType.idType && value.fieldType.idType === 'module') {
        var className = this.imports[`${_type(value.fieldType.lexeme)} `].className || 'DefaultClient';
  ...
  visitType(ast, isSubType = false) {
    if (ast.type === 'map') {
      ...
      var className = this.ctx.imports[ast.lexeme].className || 'DefaultClient';
      this.emit(`${this.ctx.imports[_name(ast)]}.${className}`);
    } else if (ast.idType === 'module') {
      let className = this.ctx.imports[ast.lexeme].className;
      if (this.ctx.conflictModels.get(className) && this.ctx.conflictModels.get(className) !== this.ctx.imports[ast.lexeme].package) {
        this.emit(`${this.ctx.imports[ast.lexeme].package}.${className}`);
      } else {
        if (!this.ctx.conflictModels.get(className)) {
          this.ctx.conflictModels.set(className, this.ctx.imports[ast.lexeme].package);
        }
        this.emit(className);
      }
