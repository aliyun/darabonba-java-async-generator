...
const { remove, save, mkdir, render, parse } = require('./util');
...
class Generator {
  ...
  async visitPom() {
    ...
    Object.keys(this.imports).forEach((key) => {
      ...
      if (dependency[1] === 'darabonba-java-core' || dependency[1].startsWith('aliyun-gateway-')) {
    ...
    if (!json.project.dependencies || !json.project.dependencies.dependency) {
