...
class Generator extends Emitter {
  ...
  visitReturn(ast, level) {
    ...
    if (ast.needCast &&
      (_name(property) === '_do' || _name(property) === 'doAsyncRequestBody' || _name(property) === 'doRequestBody')) {
  ...
  visitFieldType(value, node, modelName) {
    if (value.fieldType === 'array') {
      ...
      if (value.fieldType.idType && value.fieldType.idType === 'module') {
        var className = this.imports[`${_type(value.fieldType.lexeme)} `].className || 'DefaultAsyncClient';
  ...
  visitType(ast, isSubType = false) {
    if (ast.type === 'map') {
      ...
      var className = this.ctx.imports[ast.lexeme].className || 'DefaultAsyncClient';
