...
class Generator {
  constructor(option = {}) {
    ...
    this.config = Object.assign({
      outputDir: '',
      indent: '    ',
      clientPath: `${option.className || 'Client'}.java`
    }, option);
    assert.ok(this.config.outputDir, '`option.outputDir` should not empty');
    assert.ok(this.config.package, `Darafile -> java -> package should not empty, please add java option into Darafile.
      example:
        "java": {
    ...
    this.className = option.className || 'DefaultAsyncClient';
    this.implements = option.implements;
    this.output = '';
    this.exec = this.config.exec;
    this.editable = option.editable;
    this.classPackage = new Map();
    this.releaseVersion = this.config.releaseVersion;
    this.pomOutputDir = option.outputDir;
    this.packageInfo = option.packageInfo;
    this.baseClient = option && option.baseClient || this.package + '.Client';
    mkdir(path.join(option.outputDir, 'src/it/java', ...this.package.split('.')));
    mkdir(path.join(option.outputDir, 'src/test'));
    option.outputDir = path.join(option.outputDir, 'src/main/java', ...this.package.split('.'));
  ...
  visit(ast) {
    ...
    this.visitModule(ast, this.outputDir, true);
  }

  visitModule(ast, filepath, main) {
    ...
    for (let i = 0; i < enums.length; i++) {
      ...
      save(path.join(filepath, 'models', `${enumName}.java`), enum_generator.output);
    }
    ...
    for (let i = 0; i < models.length; i++) {
      ...
      model.codegen();
      save(path.join(filepath, 'models', `${modelName}.java`), model.output);
    }
    ...
    client.codegen();
    save(path.join(filepath, `${this.className}.java`), client.output);

    let iterator;
    for (let i = 0; i < this.iterators.length; i++) {
      if (this.iterators[i].iteratorType) {
        iterator = new IteratorGenerator(this.iterators[i], this);
        iterator.codegen();
        save(path.join(filepath, 'models', `${_name(this.iterators[i].iteratorType)}Iterator.java`), iterator.output);
      }
    }
  }
